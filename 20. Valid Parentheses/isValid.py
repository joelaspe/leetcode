...
# Intuition: Given an input string that comprises only the characters '(', ')', '{', '}', '[' and ']' we are asked to determine if the input string is valid based on if the parentheses (bracket, curly or normal) are the appropriate type and in order. We need a way to store the opening bracket and the order each opening bracket appeared in. When we start seeing the closing brackets again, look at the most recent thing added and see if it matches the expected. A stack structure would seem to work best for this purpose. 

#Approach:  Declare a list to store the record of opening brackets. Use python's dictionary data structure to store what brackets are associated with each other ([], {}, ()) Begin iterating through the input string, and check if it is an opening bracket, if it is, append it to end of recordArray. If instead a closing bracket is encountered, we compare the last added bracket in the recordArray and compare it to the closing bracket in our dictionary (i.e. if we have ']' then '[' should be on record). We also pop() from the recordArray. If at any time that comparison fails, return false. After iteration through the input string, do a final check of the tracker index, it should be at 0. If it is not, return false because we didn't fully close out the opening brackets. Otherwise return true. There is one more edge case to consider, that is if a closing bracket is received while the recordStack is empty. In this case, return false immediately.

#Complexity: 
#-Time: We loop through the input array operations once so the time complexity is $$O(n)$$
#-Space: The record list could be as large as the input array at the most, so space complexity is $$O(n)$$.  */
