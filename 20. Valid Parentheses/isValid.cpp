    ...
    bool isValid(string s) {
        ...
        for(int i = 0; i < s.length(); i++) {
            if(s[i] == '(' || s[i] == '[' || s[i] == '{') {
                ...
                //edge case, if stack is empty but we received a closing bracket, automatically return false
                ...
                switch (s[i]) {
                    ...
                    default: {
                        // received an invalid character, return false
                        return false;
                    }
                }
                // have to use top() in the switch because pop() doesn't actually return the value when popped. Once we've done our logic, then remove the last element in the stack using pop()
                recordStack.pop(); 
            }
        }
